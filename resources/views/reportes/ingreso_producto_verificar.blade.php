...
<body>
    ...
    <div class="encabezado">
        ...
        <h4 class="texto">VERIFICACIÓN/RECEPCIÓN</h4>
    ...
    <table border="1">
        <thead class="bg-principal">
            <tr>
                ...
                <th>CÓDIGO</th>
                <th>PRODUCTO</th>
                <th class="centreado">SOLICITADO</th>
                <th class="centreado">RECIBIDO</th>
                <th class="centreado">FALTANTES</th>
        ...
        <tbody>
            ...
            @foreach ($ingreso_producto->ingreso_detalles as $item)
                <tr>
                    ...
                    <td class="centreado">{{ $item->verificado }}</td>
                    <td class="centreado">{{ $item->faltantes }}</td>
                    <td class="">{{ $item->observacion }}</td>
                </tr>
                @php
                    $cant_total += (float) $item->cantidad;
                    $cant_verificado += (float) $item->verificado;
                    $cant_faltantes += (float) $item->faltantes;
                @endphp
            @endforeach
            <tr>
                <td class="bold derecha" colspan="3">TOTAL</td>
                <td class="bold centreado">{{ $cant_total }}</td>
                <td class="bold centreado">{{ $cant_verificado }}</td>
                <td class="bold centreado">{{ $cant_faltantes }}</td>
                <td></td>
            </tr>
        </tbody>
    </table>
