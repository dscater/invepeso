...
const { setMarca, limpiarMarca, form } = useMarcas();
...
const eliminarMarca = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let respuesta = await axiosDelete(route("marcas.destroy", item.id));
...
<template>
    ...
    <Content>
        <template #header>
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="m-0"><i class="fa fa-list"></i> Marcas</h3>
                </div>
                <!-- /.col -->
                <div class="col-sm-6">
